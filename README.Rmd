...
`applyf()` is a thin wrapper for `base::do.call()`.  The operator versions `%.|%` and `%|.%` are syntactic sugar.
Any of these functions is sufficing to pas values to a variadic function (one that takes on undetermined number
of arguments).  An easy example is `base::sum()`.
