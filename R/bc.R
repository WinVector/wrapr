...
#' @param convert logical, if TRUE try to convert from string type to other types
#' @return vector of values
#'
#' @seealso \code{\link{qc}}, \code{\link{qe}}, \code{\link{qae}}, \code{\link[base]{bquote}}, \code{\link{sx}}
#'
#' @examples
#'
...
bc <- function(
  ...
  if(strict) {
    ...
    if(length(intersect(indices, indices-1))) {
      ...
      stop(paste0(
        "value boundaries possibly ambiguous, not safe to return value: ",
        toks[min_index],
        toks[min_index + 1]))
    }
  }
...
#' @return vector of values
#'
#' @seealso \code{\link{qc}}, \code{\link{qe}}, \code{\link{qae}}, \code{\link[base]{bquote}}, \code{\link{bc}}
#'
#' @examples
#'
...
sx <- function(
  ...
  # check arguments
  if(!is.character(s)) {
    stop("wrapr::sx s must be character")
  }
  if(length(s)!=1) {
    stop("wrapr::sx s must be length 1")
  }
  wrapr::stop_if_dot_args(substitute(list(...)), "wrapr::sx")
  ...
  single_quote_str <- "('((\\\\.)|[^'])*')"  # with escapes
  double_quote_str <- gsub("'", '"', single_quote_str, fixed = TRUE)
  unquo_regexp <- paste0('([^ \\\'"', sep_symbols,'+-][^ \\\'"', sep_symbols,']*)')  # can overlap with number: .4
  sep_regexp <- paste0('([ ', sep_symbols, ']+)')
  pattern <- paste(
    single_quote_str,
    double_quote_str,
