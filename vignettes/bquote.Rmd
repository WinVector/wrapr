...
For string interpolation we are intentionally using the notation that Thomas Lumley’s picked in 2003 when he introduced quasi-quotation into <code>R</code> (different than string-interpolation, but the topic of our next section).
...
All <code>evalb()</code> does is: call <code>bquote()</code> as intended.  We also used the same symbol for string interpolation as the concepts are so similar having a related notation is a good idea.
