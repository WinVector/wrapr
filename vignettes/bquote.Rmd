...
For string interpolation we are intentionally using the notation that Thomas Lumley’s picked in 2003 when he introduced quasi-quotation into <code>R</code>.

This is a common need and there are many [<code>R</code>](https://www.r-project.org) packages that supply variations of such functionality:
...
<code>evalb()</code> is is a very simple function built on top of <code>base::bquote()</code>:

```{r}
print(evalb)
```

<code>evalb()</code> is likely the exact sort of application Thomas Lumley’s intended when he added <code>bquote()</code> to <code>R</code>.  All we have done above is: call <code>bquote()</code> as intended.  We also used the same symbol for string interpolation as the concepts are so similar having a related notation is a good idea.

There are other <code>R</code> quasi-quotation systems including:
...
If you don't want to wrap your <code>plot()</code> call in <code>evalb()</code> you can instead pre-adapt the function.  Below we create a new function <code>plotb()</code> that is intended to shorthand for <code>eval(bquote(plot(...)))</code>.
