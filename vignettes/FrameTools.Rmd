...
`data.frame`s.  An important use case is: building the control table for `rowrecs_to_blocks()` and `blocks_to_rowrecs()` (example [here](https://winvector.github.io/cdata/articles/cdata.html)).
...
`wrapr` supplies the method [`draw_frame`](https://winvector.github.io/wrapr/reference/draw_frame.html) which at first glance appears to be a mere pretty-printer:
...
Expressions not in parenthesis (such as "<code>3 + 2</code>") will confuse the langauge transform `build_frame()` uses to detect cell boundaries.
