...
This escape notation arises as a natural consequence of a design of `qc()` that calls `c()` instead of quoting it (i.e. delegates `c()`-expressions to `c()`). The `bquote()-.()` should be the prefered notation for regularity, and to match any other `bquote()` quasiquotation interface (such as `qchar_frame()` or [even a variation of `dplyr`](http://www.win-vector.com/blog/2018/10/quasiquotation-in-r-via-bquote/)).
...
Notice that if one had `qc()` one would not need additional special-capture methods such as `ggplot2::aes()` and `ggplot2::aes_string()`.

The `qc()` notation is very powerful and clearly indicates where which quoting rules are in effect when.
