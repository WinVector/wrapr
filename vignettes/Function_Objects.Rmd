...
And we can build such composite pipelines directly, without using `ApplyTo()` or pipes.

```{r listlist}
s2 <- new("UnaryFnList", items = list(model_o, exp_step))
cat(format(s2))
 ...
 * The `SrcFunction` class which accepts the source code for an arbitrary expression.
 * The `PartialFunction` class which directly accepts a function (including it closure).
 
Examples include:

```{r specfn}

s4 <- new("SrcFunction", expr_src = ". + y", arg_name = ".", args= list(y=13))
print(s4)
22 %.>% s4

s5 <- new("PartialFunction", fn = tan, arg_name = "x", args = list())
1:4 %.>% s5
```
