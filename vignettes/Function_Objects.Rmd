...
purpose of these pipe interfaces is to be able to treat objects as functions: i.e. to be
able to pipe data into objects.

That is: [pipe notation](https://winvector.github.io/wrapr/articles/dot_pipe.html) wants a world data transforms are single argument functions (with other
...
```{r lms3_1}
model_o <-  pkgfn("stats::predict.lm",
                  arg_name = "newdata", 
...
Many function object steps can be captured by piping them together.

```{r lst}
exp_step <- pkgfn("base::exp",
                  arg_name = "x")
...
And we can build such composite pipelines directly, without using `ApplyTo()` or pipes using the `fnlist()` method.

```{r listlist}
s2 <- fnlist(model_o, exp_step)
cat(format(s2))
...
The `steps` object is a `UnaryFnList` `S4` class instance.  This class's  purpose is to implement the `UnaryFn` interface (essentially be compatible with `ApplyTo()`) and carry a list of steps in its `@items` slot.  The idea is this object is convenient to serialize and to pipe into.
...
s4 <- srcfn(". + y", arg_name = ".", args= list(y=13))
print(s4)
22 %.>% s4

s5 <- wrapfn(tan, arg_name = "x")
print(s5)
1:4 %.>% s5
```

A `wrapr` [dot-pipe](https://winvector.github.io/wrapr/reference/dot_arrow.html) pipeable object can be converted into a single-argument function of "dot" with the [`as_fn()`](https://winvector.github.io/wrapr/reference/as_fn.html) method:
...
And that the basics of `wrapr` function objects.  For a more substantial data processing example please see [here](https://github.com/WinVector/vtreat/blob/master/extras/ModelingPipelines.md).
