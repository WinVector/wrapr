...
When I started writing about methods for better ["parametric programming" interfaces for `dplyr`](http://www.win-vector.com/blog/2016/12/parametric-variable-names-and-dplyr/) for [`R`](https://www.r-project.org) programmers in December of 2016 there were three competing primary reactions:
...
Even prior to introducing usable tools, giving a name to the problem makes it easier to intensify it as a separable step and simplify larger work-flows.
...
Let's do some medical science (instead of programming). Both the design and discussion of substitution solutions should be driven from concrete realistic use cases. This will give us a taste of what working with each solution is like in practice.
...
```
    ...
    ##       <dbl>               <chr>           <dbl>       <dbl>
    ## 1         1 withdrawal behavior               5  0.95257413
    ## 2         1 positive re-framing               2  0.04742587
    ## 3         2 withdrawal behavior               3  0.26894142
    ## 4         2 positive re-framing               4  0.73105858
...
```
    ...
    ##       <dbl>               <chr>       <dbl>
    ## 1         1 withdrawal behavior   0.9525741
    ## 2         2 positive re-framing   0.7310586
...
The `R` expert would write the following, and the occasional `R` user (with some coaching) could maintain it.
...
There is a temptation that if you are going to only teach one system it might as well be `rlang`/`tidyeval` as "that is what comes with `dplyr`". I feel this is a false savings as while `rlang`/`tidyeval` "is already in `dplyr`" the `rlang`/`tidyeval` concepts are not "already in the user" (and in fact encompass a fairly number of irregular needing to be memorized details).

Our preference is "`wrapr::let()`". "`wrapr::let()`" delivers a lot of power for a modest amount of cognitive load. Each of the above systems involves different trade-offs and compromises, and we feel one must really try a few in production before being an expert.
