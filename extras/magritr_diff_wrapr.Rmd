...
work_examples <- function(exprs, target) {
    ...
    res <- vapply(txt, 
                  function(txti) {
                    ...
                    mp <- c("!"="&#33;", "\""="&quot;", 
                            ...
                            "_"="&#95;", "`"="&#96;", "{"="&#123;",
                            "|"="&#124;", "}"="&#125;", "~"="&#126;")
                    chars[chars %in% names(mp)] <- 
                      mp[chars[chars %in% names(mp)]]
                    names(chars) <- NULL
  ...
  evals <- evals %.>%
    mutate_nse(., 
               magrittr_res = eval_expr(magrittr_expr),
               wrapr_res = eval_expr(wrapr_expr),
               magrittr_good = checcol(magrittr_res),
               wrapr_good = checcol(wrapr_res),
               magrittr_res = format(magrittr_res, digits = 3),
               wrapr_res = format(wrapr_res, digits = 3),
               magrittr_res = ifelse(magrittr_good,
                                     cell_spec(magrittr_res,
                                               "html", 
                                               color = "darkgreen", 
                                               bold = TRUE),
                                     magrittr_res),
               wrapr_res = ifelse(wrapr_good,
                                  cell_spec(wrapr_res,
                                            "html", 
                                            color = "darkgreen", 
                                            bold = TRUE),
