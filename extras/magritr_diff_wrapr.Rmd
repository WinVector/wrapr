...
Roughly they are saying `x %>% f(ARGS)` can be considered shorthand for `{ . <- x; f(., ARGS) }` where the 
evaluation in question happens in a temporary environment.

These descriptions are useful, but to master the `magrittr` piping symbol one
eventually has to work out the difference in how `magrittr` handles expressions versus functions,
and the special use of `()` (pre-evaluation) and `{}` (general expression handling) in `magirttr` pipelines.
