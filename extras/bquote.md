...
This works, as (like `strmacro()`) `let()` specifies replacements by target names (not inline annotation) and can work left-hand sides of argument bindings. We can re-work one of or `strmacro()` examples to demonstrate this.
...
Something we emphasize is: potential ambiguity between replacement targets and non-replacement is easily avoidable as both the full source code and replacement plan are written in the same code block.
