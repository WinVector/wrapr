...
# Base R data manipulation tools in more pipe friendly wrappers.
...
#' Part piping with base R series: \url{http://www.win-vector.com/blog/tag/piping-with-base-r/}.
#'
#' @param assign_columns_data_frame data.frame to work with
#' @param ...  named exprssions to add to data frame
#' @param assign_columns_env environment to work in
...
#' assign_columns(d, y = x*x, d = 0, z = x + y, d = 1, q = x + d)
#'
#' @export
#'
assign_columns <- function(assign_columns_data_frame,
                              ...,
                              assign_columns_env = parent.frame()) {
  terms <- substitute(list(...))[-1]
  if(!is.data.frame(assign_columns_data_frame)) {
    stop("assign_columns: assign_columns_data_frame must be a data.frame")
  }
  if(length(terms)<=0) {
    stop("assign_columns: expected transforms")
  }
  for(i in seq_len(length(terms))) {
    ni <- names(terms)[[i]]
    if(length(ni)<=0) {
      stop("assign_columns: empty name")
    }
    if( (!is.character(ni)) && (!is.character(ni)) ) {
      stop(paste("assign_columns: odd class for name: ", class(ni)))
    }
    ni <- as.character(ni)
    if(nchar(ni)<=0) {
      stop("assign_columns: empty name")
    ...
    ti <- terms[[i]]
    eval_env <- new.env(parent = assign_columns_env)
    assign(".d", assign_columns_data_frame, eval_env)   # data prefix .d$
    assign(".e", assign_columns_env, eval_env) # environment prefix .e$
    assign_columns_data_frame[[ni]] <- eval(ti,
                                               envir = assign_columns_data_frame,
                                               enclos = eval_env)
  }
  assign_columns_data_frame
}
